...
class Play():
    ...
    def __init__(self, how_many, imgname, filt1, filt2, filtered_aircraft):
        ...
        if imgname == "Name":
            for item in play_button_list_name:
                self.make_answer_but = Button(self.button_frame, text=item[0], command=partial(self.round_results, "Name", item[1]), width=22, height=2, 
                                            font=med_font, bg="#aaaacc", border=1, relief="solid", wraplength=200)
        ...
        self.answer_label = Label(root, text="Select the answer you think is\ncorrect and find out if you are.", font=small_font,
                            border=1, relief="solid", bg="#FFFFFF", height=3)
    ...
    def round_results(self, button_type, button_pressed):
        ...
        if user_button_name == self.ans_name:
            ...
            self.answer_label.configure(text=f"{user_button_name} is the Correct Answer!!!\nCongratulations.", bg="#55FF55")
        else:
            # If user answer is wrong
            self.answer_label.configure(text=f"{user_button_name} is not the Correct Answer...\nThe Correct Answer is {self.ans_name}.\nGood Luck Next Time!", bg="#FF5050")

        # End of Quiz
        if self.rounds_played == self.rounds_wanted:
            self.next_button.configure(state=DISABLED, text="Game Over", bg="#FF5050")
            self.end_button.configure(text="Play Again", bg="#55FF55")
